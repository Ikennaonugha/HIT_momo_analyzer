...
CREATE TABLE IF NOT EXISTS users (
  ...
  phone_number VARCHAR(20) UNIQUE NOT NULL COMMENT 'Phone number of the user associated with their MoMo account',

  id_number VARCHAR(50) UNIQUE NOT NULL COMMENT 'National Id number of the user, 1 user cannot be registered twice with the same Id number',
...
CREATE INDEX idx_transactions_sms ON transactions(sms_id);

CREATE INDEX idx_tp_transaction ON transaction_participants(transaction_id);
CREATE INDEX idx_tp_user ON transaction_participants(user_id);

# Date-based analytics
CREATE INDEX idx_transactions_date ON transactions(transaction_date);
CREATE INDEX idx_sms_date_recorded ON sms_messages(date_recorded);

# Logging/debugging indexes
...
#adding sample data in the sms_messages table
INSERT INTO sms_messages (sms_body, date_recorded, read_status) VALUES
...
# Adding sample data in the users table
INSERT INTO users (user_id, full_name, phone_number, id_number, date_of_birth) VALUES
...
# Adding sample data in the transactions table
INSERT INTO transactions (transaction_category_id, sms_id, transaction_reference, currency, amount, service_fee, transaction_status, transaction_date) VALUES
